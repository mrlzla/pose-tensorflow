...
def extract_cnn_output(outputs_np, cfg, pairwise_stats=None):
    ...
    if cfg.pairwise_predict:
        ...
        for pair in pairwise_stats:
            pair_id = (num_joints - 1) * \
                pair[0] + pair[1] - int(pair[0] < pair[1])
...
def argmax_pose_predict(scmap, offmat, stride):
    ...
    for joint_idx in range(num_joints):
        argmax = np.argmax(scmap[:, :, joint_idx])
        print('ARGMAX= ', argmax)
        maxloc = np.unravel_index(argmax,
                                  scmap[:, :, joint_idx].shape)
        offset = np.array(offmat[maxloc][joint_idx])[
            ::-1] if offmat is not None else 0
    ...
    for joint_idx in range(num_joints):
        ...
        for joint_idx_end in range(num_joints):
            if joint_idx_end != joint_idx:
                pair_id = (num_joints - 1) * joint_idx + \
                    joint_idx_end - int(joint_idx < joint_idx_end)
                difference = np.array(pairwise_diff[maxloc][pair_id])[
                    ::-1] if pairwise_diff is not None else 0
                pos_f8_end = (np.array(maxloc).astype('float') *
                              stride + 0.5 * stride + difference)[::-1]
